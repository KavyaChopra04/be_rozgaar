...
{
   int n; cin>>n;
   vector<int> numobj(n + 1);
   for(int i = 0; i< n; i++)
   {
	cin>>numobj[i];
   }
   vector<set<int>> allobj(n+1);
   for(int i = 0; i < n; i++)
   {
   ...
   int totcoll = INT64_MAX;
   for(int i = 0; i < n - 1; i++)
   {
		//Let this be the last column that moves right
		int mycoll = 0;
		for(auto x: allobj[i])
		{
			if((i + 1)< n && allobj[i + 1].find(x)!=allobj[i+1].end())
			{
				mycoll++;
			}
			if((i + 2) < n && allobj[i + 2].find(x)!=allobj[i+2].end())
			{
				mycoll++;
			}
		}
		if(i>=1){
			for(auto x: allobj[i-1])
			{
				if((i + 1) < n && allobj[i + 1].find(x)!=allobj[i+1].end())
				{
					mycoll++;
