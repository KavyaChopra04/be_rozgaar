...
void solve()
{
    ...
    {
        int indct = ind +1;
        int prev_val = v[ind].first;
        vector<int> interv = {v[ind].second};
        while(indct < v.size())
        {
            if(v[indct].first != prev_val + 1) break;
            interv.pb(v[indct].second);
            prev_val = v[indct].first;
            indct++;
        }
        cons.pb(interv);
        ind = indct;
    ...
    for(auto x: cons)
    {
        maxval = max(maxval, maxksubarrsum(x, k));
    }
    cout<<maxval<<'\n';
