...
struct segtree {
    ...
    void set(int index_in_arr, int value, int search_at_index, int l, int r)
    {
        assert(l<=r);
        if(r == l + 1){
            tree[search_at_index] = value;
        }
        else{
            int midpt = (l+r)/2;
            if(index_in_arr < midpt)
            {
                set(index_in_arr, value, 2*search_at_index + 1, l, midpt);
            }
            else
            {
                set(index_in_arr, value, 2*search_at_index + 2, midpt, r);
            }
        }
        tree[search_at_index] = tree[2*search_at_index + 1] + tree[2*search_at_index + 2];
    }
    ...
    int sum(int l, int r, int x, int lx, int rx){
        ...
        int midpt = (lx + rx)/2;
        return sum(l, r, 2*x+1, lx, midpt) + sum(l, r, 2*x+2, midpt, rx);
    }
    int sum(int l, int r)
    {
