...
double calculateTime(const vector<int> &pos, const vector<int> &times, double meetpos)
{
    double ans = 0;
    for(int i=0;i<pos.size();i++)
    {
        ans = max(ans, times[i] + abs(pos[i] - meetpos));
    }
    return ans;
}
void solve()
{
    ...
    vector<map<pair<int, char>, int>> v(n + 1);
    v[0] = m;
    for(int i=0; i<n; i++)
    {
        m[mp(i%3, s[i])]++;
    ...
    vector<string> allcycles = {"abc", "acb", "bac", "bca", "cab", "cba"};
    while(q--)
    {
        int a,b;
        cin>>a>>b;
        auto tup = returnVals(a-1, b-1);
        ...
        int ans = INT_MAX;
        for(auto s: allcycles)
        {
            ans = min(ans, getdiff(tup, v[b], v[a-1], s));
        }
        cout<<ans<<'\n';
