...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if(s.empty())
        {
            s.insert(mp(heights[i], money[i]));continue;
        }
        auto it = lower_bound(s.begin(), s.end(), mp(heights[i], (int)0));
        if(it==s.begin() && it->first > heights[i])
        {
            s.insert(mp(heights[i], money[i]));
            int val = money[i];
            // P(val);
            it = upper_bound(s.begin(), s.end(), mp(heights[i] + 1, (int)0));
        ...
        else if(it==s.begin() && it->first == heights[i])
        {
            s.insert({heights[i], max(money[i], it->second)});continue;
        }
        // else if(it==s.begin())
        // {
        ...
        // }
        it--;
        s.insert(mp(heights[i], it->second + money[i]));
        int val = it->second + money[i];
        // P(val);
        it = upper_bound(s.begin(), s.end(), mp(heights[i] + 1, (int)0));
        vector<pair<int, int>> v;
        while(it != s.end())
