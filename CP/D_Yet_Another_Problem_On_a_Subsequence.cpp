...
int calcdist(int loc, int keyloc, int off)
{
	if((loc<=keyloc && keyloc<=off) || (loc>=keyloc && keyloc>=off)) return abs(loc-off);
	return (abs(loc-keyloc) + abs(keyloc-off));
}
void solve()
{
    int n;cin>>n;
    ...
    vector<vector<int>> goodarr(n+1, vector<int>(n+1, 0));
    for(int i=1;i<=n;i++)
    {
        for(int j=1;j+i<=n;j++)
        {
            if(arr[j]<=0 || arr[j]>=n){//cout<<"h1"<<endl;
            goodarr[j][j+i] = goodarr[j+1][j+i];}
            else if(arr[j]==1){//cout<<"h2"<<endl;
            goodarr[j][j+i] = (1 + goodarr[j+1][j+i])%mod;}
            else if(arr[j]>=2 && arr[j]<=n+1){//cout<<"h3"<<endl;
				//cout<<"combi coeff is "<<cc[i-1][arr[j]-1]<< " top "<<i-1<<" bot "<<arr[j]-1<<endl;
				goodarr[j][j+i] = (cc[i-1][arr[j]-1] + goodarr[j+1][j+i])%mod;}
            //cout<<j<<" "<<j + i<<" "<<goodarr[j][j+i]<<endl;
        }
    }
    //cout<<endl;
    vector<int> dp(n+1, 0);
    ...
    for(int i=1;i<=n;i++)
    {
        ...
        for(int j=0;j<i;j++)
        {
            ans= (ans + (dp[j] * goodarr[j+1][i])%mod)%mod;
        }
        dp[i] = ans;
