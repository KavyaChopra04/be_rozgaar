...
void solve()
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		for(int j=1;j<=m;j++)
		{
			if(mindp[j-1]!=-1){newdp[j] = max(mindp[j-1], calcdist(loc[i], keys[j], o));}
			else{newdp[j]=-1;}
		}
		vector<int> newmindp(m+1, -1);
		newmindp[0] = -1;
		for(int j=1;j<=m;j++)
		{
			if(newmindp[j-1]!=-1){newmindp[j] = min(newmindp[j-1], newdp[j]);}
			else{newmindp[j] =newdp[j];}
		}
		dp=newdp;
		mindp=newmindp;
		// cout<<i<<endl;
