...
void solve()
{
    int n,m,k;
    cin>>n>>m>>k;
    vector<vector<int>> r_nw(n, vector<int>(m));
    vector<vector<int>> l_nw(n, vector<int>(m));
    vector<vector<int>> u_nw(n, vector<int>(m));
    vector<vector<int>> d_nw(n, vector<int>(m));
    for(int i=0;i<n;i++)
    {
    ...
    for(int steps=1;steps<=k;steps++){
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<m;j++)
            {
                if(steps%2==1 || steps<2)
                {
                    dp[i][j][steps]=INT_MAX;
                }
                else{
                    dp[i][j][steps] = INT_MAX;
                    auto neighbours = get_neighbours(i, j, n,m,r_nw,l_nw,d_nw, u_nw);
                    //cout<<"neighbours for "<<i<<" "<<j<<endl;
                    for(auto nb: neighbours)
                    {
                        if(dp[get<0>(nb)][get<1>(nb)][steps-2]!=INT_MAX)
                        //cout<<get<0>(nb)<<" "<<get<1>(nb)<<" "<<get<2>(nb)<<endl;
                        dp[i][j][steps] = min(dp[i][j][steps], dp[get<0>(nb)][get<1>(nb)][steps-2] + 2*get<2>(nb));
                    }
                } 
            }
        ...
        //     for(int j=0;j<m;j++)
        //     {
        //         if(dp[i][j][steps]!=INT_MAX){cout<<dp[i][j][steps]<<" ";}
        //         else{cout<<-1<<" ";}
        //     }
        //     cout<<'\n';
        // }
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            if(dp[i][j][k]!=INT_MAX){cout<<dp[i][j][k]<<" ";}
            else{cout<<-1<<" ";}
        }
        cout<<'\n';
    }
