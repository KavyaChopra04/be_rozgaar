...
int fillDP(const vector<vector<int>> &comp, int point, int mask, int n)
{
    if(point==n)
    {
        if(mask == (1<<point)-1)
    ...
    {
        int k = 1<<i;
        if(comp[point][i] && !(mask & k ))
        {
            ans=(ans + fillDP(comp, point+1, mask | k, n))%mod;
        }
    }
    return dp[point][mask] = ans;
}
