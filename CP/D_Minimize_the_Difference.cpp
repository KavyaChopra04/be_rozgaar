...
bool works(vector<int> arr, int k, int min_avg, int sum)
{
    int suff_sum = sum;
    // arr[1] = arr[1] + arr[0] - min_avg;
    // arr[0] = min_avg;
    // suff_sum = sum - arr[1] - arr[0];
    for(int i=arr.size()-1;i>=0;i--)
    {
        if(arr[i] < min_avg)
        {
            arr[i-1] = arr[i] + arr[i-1] - min_avg;
            arr[i] = min_avg;
        }
    }
    for(int i=0;i<arr.size()-1;i++)
    {
        if(arr[i] > min_avg + k)
        {
            arr[i+1] = arr[i] + arr[i+1] - (min_avg + k);
            arr[i] = min_avg + k;
        }
    ...
    return (arr[arr.size()-1] <= min_avg + k && arr[arr.size()-1]>=min_avg);
}
void solve()
{
    int n;
    cin>>n;
    vector<int> arr(n);
    int min_avg = INT64_MAX;
    int sum = 0;
    int min_avg_ind = -1;
    int sum_till_min_ind = 0;
    for(int i=0;i<n;i++)
    {
        cin>>arr[i];
        sum += arr[i];
        if(min_avg > sum/(i+1)){min_avg = sum/(i+1); min_avg_ind = i;sum_till_min_ind=sum;}
    }
    for(int i=0;i<=min_avg_ind;i++)
    {
        arr[i] = min_avg;
    }
    for(int i=min_avg_ind;i>=0;i--)
    {
        if(sum_till_min_ind>(min_avg*(min_avg_ind+1))) {arr[i]+=1;}
        sum_till_min_ind--;
    }
    // cout<<"printing arr\n";
    // for(int i=0;i<n;i++)
    ...
    if(n==1) {cout<<0<<'\n';return;}
    int lo=0;
    int hi=1000000000000;
    //cout<<"min_avg is "<<min_avg<<endl;
    while(lo<hi)
    {
        int mid = lo + (hi-lo)/2;
        // cout<<"k is "
        if(works(arr, mid, min_avg, sum))
        {
            hi=mid;
        }
        else
