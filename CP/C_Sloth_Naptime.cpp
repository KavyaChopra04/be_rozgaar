...
void solve()
{
    ...
    while(q--)
    {
        int a, b, c;
        cin>>a>>b >> c;
        int lcanode = getLCA(a, b, dp, anc);
        // cout<<lcanode<<'\n';
        if(c >= dp[a] + dp[b] - 2*dp[lcanode])
        {
            cout<<b<<'\n';
            continue;
        }
        else if(c <= dp[a] - dp[lcanode])
        {
            cout<<getParent(a, c, anc)<<'\n';
        }
        else
        {
            // cout<<dp[a] + dp[b] - 2*dp[lcanode] - c<<'\n';
            cout<<getParent(b, dp[a] + dp[b] - 2*dp[lcanode] - c, anc)<<'\n';
        }
    }
