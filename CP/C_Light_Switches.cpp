...
vector<pair<int, int>> intersectIntervals(const vector<pair<int, int>> &v1, const vector<pair<int, int>> &v2, int k)
{
    ...
    for(auto p1: v1)
    {
        for(auto p2: v2)
        {
            ...
            // if(p2.second > 4*k && p1.first < k)
            // {
            //     p2.first = 0;
            //     p2.second = p2.second % (2*k);
            ...
            else
            {
                ...
                {
                    answers.pb({p1.first, p2.second});
                }
                else
                {
                    answers.pb({p2.first, p1.second});
                }
            }
        }
    }
...
pair<bool, int> minIntersection(const vector<vector<pair<int, int>>> &intervals, int k)
{
    if(intervals.size() == 1)
    {
        return {true, intervals[0][0].first};
    }
    ...
    //     cout<<endl;
    // }
    vector<pair<int, int>> prelim = intersectIntervals(intervals[0], intervals[1], k);
    // for(auto x: prelim)
    // {
    //     cout<<"("<<x.first<<", "<<x.second<<"),  ";
    // }
    ...
    for(int i=2; i<intervals.size();i++)
    {
        ...
        prelim = intersectIntervals(prelim, intervals[i], k);
        // for(auto x: prelim)
        // {
        //     cout<<"("<<x.first<<", "<<x.second<<"),  ";
        // }
...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        cin>>v[i];
        modv[i] = v[i] % (2*k);
        // if(s.find(modv[i] - k)!=s.end() || s.find(modv[i] + k)!=s.end())
        // {
        //     imposs = true;
        // }
        s.insert(modv[i]);
    ...
    for(auto x: s)
    {
        // if(x <= k)
        // {
            intervals.pb({{x, x + k-1}, {x + 2*k, x + 3*k -1}});
        // }
        // else
        // {
        //     intervals.pb({{x, x + k-1}});
        // }
    }
    pair<bool, int> x = minIntersection(intervals, k);
    if(!x.first)
    {
        cout<<-1<<'\n';return;
    }
    else
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(x.second%(2*k) >= v[i]%(2*k))
            {
                ans = max(ans, v[i] - v[i]%(2*k) + x.second%(2*k));
            }
            else
            {
                ans = max(ans, v[i] - v[i]%(2*k) + x.second%(2*k) + 2*k);
            }
            // cout<<ans<<'\n';
        }
