...
int dfs(vector<vector<int>> &adj, vector<vector<int>> &dp, int n, int parent, int index)
{
    if(dp[n][index]!=-1) return dp[n][index];
    if(adj[n].size()==1 && parent!=-1)
    {
        //cout<<"node "<<n<<" is a leaf"<<endl;
        return dp[n][index] = 0;
    }
    dp[n][index] = 0;
    int ans=0;
    if(index==0)
    {
        for(auto node : adj[n])
        {
            ...
            ans+=max(dfs(adj, dp, node, n, 0), dfs(adj, dp, node, n, 1));
        }
        //cout<<"computed answer "<<ans<<" for node "<<n<<" at index "<<index<<endl;
    }
    else
    {
        int ansv = INT_MIN;
        ...
        for(auto node: adj[n])
        {
            if(node==parent) continue;
            int ansc=ans;
            ansc = ansc - max(dfs(adj, dp, node, n, 0), dfs(adj, dp, node, n, 1)) + 1 + dfs(adj, dp, node, n, 0);
            ansv = max(ansc, ansv);
        }
        ans = ansv;
        //cout<<"computed answer "<<ans<<" for node "<<n<<" at index "<<index<<endl;
...
void solve()
{
    ...
    cout<<max(dfs(adj, dp, 1, -1, 0),dfs(adj, dp, 1, -1, 1)) <<'\n';
}
signed main() {
