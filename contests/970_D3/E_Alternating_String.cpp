...
void solve()
{
    ...
    vector<vector<int>> pre_counts(n+2, vector<int>(26, 0));
    vector<vector<int>> post_counts(n+2, vector<int>(26, 0));
    pre_counts[1][s[0]-'a']=1;
    post_counts[n][s[n-1]-'a']=1;
    for(int i=1;i<n;i++)
    {
        pre_counts[i+1] = pre_counts[i-1];
        pre_counts[i+1][s[i]-'a']+=1;
    }
    for(int i=n-1;i>=1;i--)
    {
        post_counts[i] = post_counts[i+2];
        post_counts[i][s[i-1]-'a']+=1;
    }
    post_counts[0]=post_counts[2];
    pre_counts[n+1]=pre_counts[n-1];
    if(n%2==0)
    {
        int max_eve = *std::max_element(pre_counts[n+1].begin(), pre_counts[n+1].end());
        int max_odd = *std::max_element(pre_counts[n].begin(), pre_counts[n].end());
        cout<<n-max_eve-max_odd<<'\n';
    }
    else
    {
        int ans=INT_MAX;
        int max_eve = *std::max_element(pre_counts[n-1].begin(), pre_counts[n-1].end());
        int max_odd = *std::max_element(pre_counts[n-2].begin(), pre_counts[n-2].end());
        ans = min(ans, n-max_eve-max_odd);
        max_eve = *std::max_element(post_counts[2].begin(), post_counts[2].end());
        max_odd = *std::max_element(post_counts[3].begin(), post_counts[3].end());
        ans=min(ans, n-max_eve-max_odd);
        for(int i=1;i<=n-2;i++)
        {
            vector<int> total_counts_odd(26, 0);
            vector<int> total_counts_even(26, 0);
            for(int j=0;j<26;j++)
            {
                total_counts_odd[j]=post_counts[i+2][j]+pre_counts[i-1][j];
                total_counts_even[j]=post_counts[i+3][j]+pre_counts[i][j];
                int max_eve = *std::max_element(total_counts_odd.begin(), total_counts_odd.end());
                int max_odd = *std::max_element(total_counts_even.begin(), total_counts_even.end());
                ans = min(ans, n-max_eve-max_odd);
            }
